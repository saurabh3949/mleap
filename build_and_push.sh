...
local_image=combustml/mleap-serving
version=$(sbt "inspect version" | grep Setting: | awk -F" = " '{print $2}' | sed 's/.*$//')

image=im-mleap-serving
...
docker tag "${local_image}:${version}" ${fullname}
